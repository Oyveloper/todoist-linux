...
.PHONY: push-release
push-release:
	github-release upload --owner krydos --repo todoist-linux --token ${GITHUB_TOKEN} --tag "${CIRCLE_TAG}" --name "Todoist Linux ${CIRCLE_TAG}" dist/Todoist* dist/todoist-linux.zip
